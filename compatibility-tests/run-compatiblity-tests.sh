...
(
    ...
    npm install /data/src/node --SMRF_LIB_DIR=$CPP_INSTALL_PREFIX/lib64 --SMRF_INCLUDE_DIR=$CPP_INSTALL_PREFIX/include
    npm install
)
...
(
    ...
    for MODE in "${COMPRESSION[@]}"
    do
        npm run-script serializer --compatibility-test:path=$NODE_OUTPUT_DIR/$MODE --compatibility-test:compressed=$MODE
    done
)
